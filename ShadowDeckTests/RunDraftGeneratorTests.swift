...
final class RunDraftGeneratorTests: XCTestCase {
    ...
    // MARK: - Live local Missions folder (optional)

    /// When the developer has SRM PDFs at the usual path, assert the heuristic
    /// produces usable drafts (mission code, player-facing text, payouts).
    func testLiveLocalMissionsPDFsIfPresent() throws {
        // Prefer the real user Documents path (not the app container sandbox home).
        let candidates = [
            URL(fileURLWithPath: "/Users/zdavis/Documents/eBooks/Shadowrun/Missions"),
            URL(fileURLWithPath: NSString(string: "~/Documents/eBooks/Shadowrun/Missions").expandingTildeInPath)
        ]
        guard let dir = candidates.first(where: { FileManager.default.fileExists(atPath: $0.path) }) else {
            throw XCTSkip("No local Missions folder at \(candidates[0].path)")
        }
        let pdfs = try FileManager.default.contentsOfDirectory(at: dir, includingPropertiesForKeys: nil)
            .filter { $0.pathExtension.lowercased() == "pdf" }
            .sorted { $0.lastPathComponent < $1.lastPathComponent }
        guard !pdfs.isEmpty else {
            throw XCTSkip("Missions folder has no PDFs")
        }
        ...
        for url in pdfs {
            ...
            do {
                ...
                if draft.missionCode == nil {
                    failures.append("\(name): missing mission code")
                }
                if draft.playerFacingSummary.count < 40 {
                    failures.append("\(name): thin playerFacing (\(draft.playerFacingSummary.count) chars)")
                }
                if draft.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                    failures.append("\(name): empty title")
                }
                // Soft diagnostics (don’t fail build on missing nuyen for odd PDFs).
                print(
                    """
                    [SRM live] \(name)
                      title=\(draft.title)
                      code=\(draft.missionCode ?? "—")
                      client=\(draft.client.isEmpty ? "—" : draft.client)
                      location=\(draft.location.isEmpty ? "—" : draft.location)
                      objectives=\(draft.objectives.count)
                      ¥=\(draft.expectedPayoutNuyen.map(String.init) ?? "—") karma=\(draft.expectedKarma.map(String.init) ?? "—")
                      playerFacing=\(draft.playerFacingSummary.count)c risks=\(draft.knownRisks.count)c opp=\(draft.oppositionSummary.count)c
                    """
                )
            } catch {
                failures.append("\(name): extract error \(error.localizedDescription)")
            }
        }
